...
requiresApi(21);
let bridgeHandler  = require(files.cwd() + "/webview/handler/bridgeHandler.js");
let webErrorHandler = require(files.cwd() + "/webview/handler/webErrorHandler.js");
let jsBridge = files.read("webview/core/jsBridge.ts");
// let vConsole = files.read("expand/core/vConsole.ts");
let vConsole = files.read("webview/core/vconsole.min.ts");
/**
 * 初始化，支持页面加载完时注入脚本
...
function init(webViewWidget, jsFileList, supportVConsole) {
    ...
    webViewWidget.webViewClient = new JavaAdapter(android.webkit.WebViewClient, {
        ...
        shouldOverrideUrlLoading: (webView, curUrl) => {
            // console.log(curUrl, '---------------------------------------------------', webView.url);
            let url = webView.url;//(request.a && request.a.a) || (request.url);
